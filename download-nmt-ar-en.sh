...
# 
dash=-
sh get-data-para.sh $SRC$dash$TGT
...
if [ "$SRC" == "ar" -a "$TGT" == "en" ]; then
  PARA_SRC_TRAIN=$PROC_PATH/$SRC-$TGT.$SRC.train
  PARA_TGT_TRAIN=$PROC_PATH/$SRC-$TGT.$TGT.train
  PARA_SRC_VALID=$PROC_PATH/$SRC-$TGT.$SRC.valid
  PARA_TGT_VALID=$PROC_PATH/$SRC-$TGT.$TGT.valid
  PARA_SRC_TEST=$PROC_PATH/$SRC-$TGT.$SRC.test
  PARA_TGT_TEST=$PROC_PATH/$SRC-$TGT.$TGT.test
fi
...
if [ "$SRC" == "en" -o "$TGT" == "en" ]; then
  ...
  mkdir -p $MONO_PATH/en
  cd $MONO_PATH/en
  wget -c http://opus.nlpl.eu/download.php?f=GlobalVoices/v2017q3/mono/en.txt.gz
fi
...
if [ "$SRC" == "ar" -o "$TGT" == "ar" ]; then
  ...
  cd $MONO_PATH/ar
  wget -c http://opus.nlpl.eu/download.php?f=GlobalVoices/v2017q3/mono/ar.txt.gz
fi
...
#   cat $(ls $SRC/download*$SRC* | grep -v gz) | head -n $N_MONO > $SRC_RAW
# fi
# if ! [[ -f "$TGT_RAW" ]]; then
#   echo "Concatenating $TGT monolingual data..."
#   cat $(ls $TGT/download*$TGT* | grep -v gz) | head -n $N_MONO > $TGT_RAW
# fi
