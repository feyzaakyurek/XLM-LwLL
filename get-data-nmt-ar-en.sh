...
# Usage:
# ./get-data-nmt.sh --src ar --tgt en --reload_codes codes_enar --reload_vocab vocab_enar
#
...
if [ "$SRC" != "de" -a "$SRC" != "en" -a "$SRC" != "fr" -a "$SRC" != "ro" -a "$SRC" != "ar" ]; then echo "unknown source language"; exit; fi
...
DATA_PATH=$PWD/data/data17langmlm
...
#   mkdir -p $MONO_PATH/en
#   cd $MONO_PATH/en
#   wget -c http://opus.nlpl.eu/download.php?f=MultiUN/v1/mono/MultiUN.en.gz
# fi
...
#   cd $MONO_PATH/ar
#   wget -c http://opus.nlpl.eu/download.php?f=MultiUN/v1/mono/MultiUN.ar.gz
# fi
